import{d as E}from"./index-PPLHz8o0.js";const{useMemo:b,useEffect:k}=__STORYBOOK_MODULE_PREVIEW_API__,{global:h}=__STORYBOOK_MODULE_GLOBAL__,{logger:B}=__STORYBOOK_MODULE_CLIENT_LOGGER__;var u="backgrounds",{document:i,window:S}=h,x=()=>S.matchMedia("(prefers-reduced-motion: reduce)").matches,L=(r,e=[],n)=>{if(r==="transparent")return"transparent";if(e.find(o=>o.value===r))return r;const a=e.find(o=>o.name===n);if(a)return a.value;if(n){const o=e.map(s=>s.name).join(", ");B.warn(E`
    ...
    `},[t]);return k(()=>{const m=e.viewMode==="docs"?`addon-backgrounds-grid-docs-${e.id}`:"addon-backgrounds-grid";if(!s){M(m);return}w(m,$)},[s,$,e]),r()},R=[C,T],G={[u]:{grid:{cellSize:20,opacity:.5,cellAmount:5},values:[{name:"light",value:"#F8F8F8"},{name:"dark",value:"#333333"}]}},H={[u]:null};export{R as decorators,H as globals,G as parameters};
