...
function prepareStory(storyAnnotations, componentAnnotations, projectAnnotations) {
  ...
  const undecoratedStoryFn = (context) => render(context.args, context);
...
var StoryStore = class {
  ...
  async loadAllCSFFiles({ batchSize = EXTRACT_BATCH_SIZE } = {}) {
    const importPaths = Object.entries(this.storyIndex.entries).map(([storyId, { importPath }]) => [
      importPath,
      storyId
    ]);
    const loadInBatches = async (remainingImportPaths) => {
      if (remainingImportPaths.length === 0)
        return Promise.resolve([]);
      const csfFilePromiseList = remainingImportPaths.slice(0, batchSize).map(async ([importPath, storyId]) => ({
        importPath,
...
var StoryRender = class {
  ...
  async render({
    ...
    try {
      ...
      const onError = (event) => unhandledErrors.add("error" in event ? event.error : event.reason);
...
var Preview = class {
  ...
  async extract(options2) {
    ...
    return this.storyStoreValue.extract(options2);
  }
  // UTILITIES
...
var PreviewWithSelection = class extends Preview {
  ...
  async renderSelection({ persistedArgs } = {}) {
    ...
    if (isStoryRender(render)) {
      invariant(!!render.story);
      const { parameters, initialArgs, argTypes, unmappedArgs } = this.storyStoreValue.getStoryContext(render.story);
...
var parseArgsParam = (argsString) => {
  ...
  return Object.entries(import_qs2.default.parse(parts.join(";"), QS_OPTIONS)).reduce((acc, [key2, value2]) => {
...
var getQueryString = ({
  ...
  const { path, selectedKind, selectedStory, ...rest } = import_qs2.default.parse(search, {
...
var getSelectionSpecifierFromPath = () => {
  const query = import_qs2.default.parse(document3.location.search, { ignoreQueryPrefix: true });
...
var WebView = class {
  constructor() {
    this.testing = false;
    const { __SPECIAL_TEST_PARAMETER__ } = import_qs2.default.parse(document22.location.search, {
      ignoreQueryPrefix: true
    });
